...
class TestMCPOAuth:
                            ...
                            with patch("os.path.exists") as mock_exists:
                                with patch("os.makedirs") as mock_makedirs:
                                    ...
                                    async def mock_async_run(coro):
                                        ...
                                        with patch(
                                            "src.mcp_manager.secrets.token_urlsafe",
                                            return_value="test-state",
                                        ):
                                            with patch(
                                                "src.mcp_manager.ClientSession",
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data='{"access_token": "existing-token", "expires_at": 9999999999}'
        ),
    )
    ...
    def test_connect_oauth_server_existing_token(
        ...
        async def mock_async_run(coro):
            ...
            with patch(
                "src.mcp_manager.ClientSession", return_value=mock_session_context
            ):
                return await coro
    ...
    @patch(
        "builtins.open",
        mock_open(read_data='{"access_token": "expired-token", "expires_at": 1000}'),
    )
    @patch("os.path.exists")
    def test_connect_oauth_server_expired_token(
        ...
        mock_input.return_value = (
            "http://localhost:8080/callback?code=refresh-code&state=test-state"
        )
        ...
        async def mock_async_run(coro):
            ...
            # Patch the state generation to return a known value
            with patch(
                "src.mcp_manager.secrets.token_urlsafe", return_value="test-state"
            ):
                with patch(
                    "src.mcp_manager.ClientSession", return_value=mock_session_context
                ):
    ...
    @patch("os.makedirs")
    async def test_save_oauth_token(
        self, mock_makedirs, mock_file, oauth_server_config
    ):
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data='{"access_token": "stored-token", "expires_at": 9999999999}'
        ),
    )
    @patch("os.path.exists")
    ...
    async def test_oauth_redirect_handler(self, oauth_server_config):
        ...
        redirect_url = await manager._handle_oauth_redirect(
    ...
    async def test_oauth_callback_handler(self, oauth_server_config):
        ...
        with patch(
            "builtins.input",
            return_value="http://localhost:8080/callback?code=auth-code&state=test-state",
        ):
    ...
    def test_oauth_config_validation(self, mock_run):
        ...
        config.get_server = lambda name: (
            config.servers[0] if name == "invalid-oauth" else None
        )

        manager = MCPManager(config)

        # Mock asyncio.run to raise the expected error
        mock_run.side_effect = MCPManagerError(
            "OAuth configuration missing required fields"
        )
    ...
    @patch("builtins.open")
    @patch("src.mcp_manager.httpx.AsyncClient")
    async def test_oauth_token_in_requests(
        self, mock_httpx_client, mock_file, mock_exists, oauth_server_config
    ):
        ...
        with patch("src.mcp_manager.ClientSession") as mock_client_class:
            with patch("src.mcp_manager.streamablehttp_client") as mock_http_client:
                ...
                result = await manager.call_tool(
                    "oauth-server", "test-tool", {"arg": "value"}
                )

        assert result["result"] == "success"
        mock_session.call_tool.assert_called_once_with(
            "test-tool", arguments={"arg": "value"}
        )
