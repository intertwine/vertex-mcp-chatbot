...
class TestScrollableResponse:
    ...
    def test_long_response_triggers_scrollable_display(self):
        ...
        with patch('src.chatbot.Console') as mock_console_class:
            ...
            with patch.object(self.chatbot, '_display_scrollable_content') as mock_scrollable:
    ...
    @patch('src.chatbot.Application')
    def test_buffer_creation_and_content(self, mock_app_class, mock_buffer_class, mock_document_class):
        ...
        document_calls = [call.args[0] for call in mock_document_class.call_args_list if call.args]
        assert rendered_content in document_calls, f"Expected {rendered_content} in Document calls: {document_calls}"
    ...
    def test_help_text_display(self):
        ...
        with patch('src.chatbot.Application') as mock_app_class:
            ...
            print_calls = [call.args[0] for call in self.chatbot.console.print.call_args_list]
            help_messages = [call for call in print_calls if "scroll" in call.lower() or "arrow" in call.lower()]
...
class TestScrollableHistory:
    ...
    def test_empty_history_displays_normally(self):
        ...
        self.chatbot.console.print.assert_called_with("[dim]No conversation history yet[/dim]")
    ...
    def test_short_history_displays_normally(self):
        ...
        self.chatbot.client.get_chat_history.return_value = [mock_item1, mock_item2]
    ...
    def test_long_history_triggers_scrollable_display(self):
        ...
        with patch('src.chatbot.Console') as mock_console_class:
            ...
            with patch.object(self.chatbot, '_display_scrollable_content') as mock_scrollable:
                ...
                # Should call scrollable display
                mock_scrollable.assert_called_once_with(long_content, "Conversation History")
